...
2. **Comparators.** Recompute the human test–retest benchmark on the same test people and items as the model. The paper's **81.72%** is the full-sample published figure, not necessarily this split; it is a short-term empirical benchmark, not a mathematical ceiling. Trivial baselines: uniform random, train majority, and **copy-last**. Copy-last uses the same earlier-versus-later pairs as the human benchmark. Staged ablations: question-only, summary-only, summary plus retrieval (frozen), and a tabular neural baseline. The paper's twin scores (**71.72%**, ratio **87.67%**) are published citations only; I did not recompute them on this split, so they are not a row in my result table.
...
4. **Leakage.** Repeating the same question text on wave 4 is not a leak. Putting the **wave-4 answer** into the model input is. Example for pid=1 / `QID154`: the person first answered **70**, then **82** on wave 4. `full_persona` already stores **82**, so that field cannot be used as the persona. `wave4_Q_wave4_A` is the held-out question plus that same **82**; if those `Answers` are not stripped, the prompt contains the target. The earlier **70** is history, not a future label: withhold it in no-copy, allow it in full-history. One item per sequence.
