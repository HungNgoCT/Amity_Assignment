...
1. **Metrics.** The system headline is the paper's **mean absolute deviation (MAD) accuracy** averaged across the 17 tasks, not a single exact-match average. A 0–100 slider and a yes/no item sit on different scales and should not be treated as equivalent. Supporting metrics stay task-specific: single-label Multiple Choice (MC) uses accuracy plus Cohen's kappa (κ; Cohen, 1960); multi-select uses Jaccard, with per-option F1 score (harmonic mean of precision and recall) as a diagnostic; matrix uses MAD plus ordinal mean absolute error (MAE); slider uses MAD plus native-scale MAE rather than exact match; unbounded anchoring uses MAD after train-only deciles. Exclude Display/Instruction (DB) screens. Join types through the catalog's `csv_columns`.
...
2. **Comparators.** I would score every system on the same test people and items.
   ...
   - **Copy-last** predicts wave 4 = the earlier answer. On a fixed repeated-item set this is the same MAD calculation as human test–retest, read as a trivial model baseline.
   - **Other trivial baselines:** uniform random, and the most common train answer (majority).
   - **Ablations:** question-only, summary-only, summary plus retrieval without fine-tuning, a small tabular neural net, and persona-shuffling. If shuffling people does not drop the score, that suggests the model is not using individual information.
...
3. **Split.** Person split, seed `20250319`, **70% / 15% / 15%** for train, validation, and test. Score only assigned non-null cells. Fit deciles and majority on train, tune on validation, and evaluate test once after the protocol is locked. Report no-copy and full-history as two separate tables; never pool them.
...
Join columns to types via `csv_columns`, never `startswith(QuestionID)`. Single Answer Vertical / Horizontal (SAVR/SAHR) are single-choice layouts; Multiple Answer Vertical / Horizontal (MAVR/MAHR) are multi-select. Counts are from the exploratory data analysis (EDA).
...
| Slider (`QID154`, `QID156`, `QID290_*`) | 12 overlap cols | MAD; MAE in 0–100 | Pearson *r*; intraclass correlation coefficient ICC(A,1) (McGraw & Wong, 1996) | Exact-match |
...
**Invalid outputs.** Freeze the scored `(pid, column)` rows before loading predictions. Missing ground truth is excluded, but an unparseable or illegal output stays in the denominator and scores headline MAD 0. Passing invalid predictions to the official script as Not a Number (`NaN`) would drop them from the denominator.
...
- Optional American Community Survey (ACS)-motivated age and education slices are diagnostics, not gates.
...
- Strip `Answers` / `Values` / `Selected*` from every question JavaScript Object Notation (JSON) payload. Do not use `full_persona`.
...
| **Take-home POC (&lt;0.5B slice)** | Leakage gate passes; MC exact match is the slice headline; slice MAD, a trivial baseline, and parse rate are reported | Leakage failure, no baseline comparison, or parse rate omitted |
