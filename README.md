...
For the POC, I would score the same leakage-safe slice with the official 17-task script and participant-bootstrap intervals. A model strictly smaller than `0.5B` parameters would count only if its paired interval over train-majority stayed above zero.